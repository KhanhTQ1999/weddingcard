google.maps.__gjsload__("geometry", function (_) {
  var Zoa = function (a, b) {
    ...
    $oa = function (a, b, c, d, e) {
      if (!d) {
        c = Zoa(a.lng(), c) / Zoa(a.lng(), b.lng());
        if (!e)
          return (
            (e = Math.sin(_.pl(a.lat()))),
            (e = Math.log((1 + e) / (1 - e)) / 2),
            (b = Math.sin(_.pl(b.lat()))),
            _.ql(
              2 *
                Math.atan(
        ...
        return e
          .fromPointToLatLng(
            new _.Fo(a.x + c * (b.x - a.x), a.y + c * (b.y - a.y)),
          )
          .lat();
      }
      e = _.pl(a.lat());
    ...
    bpa = function (a, b, c) {
      ...
      for (let r = 0, t = g.getLength(); r < t; ++r) {
        ...
        for (let C = 0; C < y; ++C) {
          ...
          if (
            (k =
              Math.abs(_.nm(k - m, -180, 180)) <= 1e-9 &&
              (Math.abs(_.nm(k - d, -180, 180)) <= c ||
                Math.abs(_.nm(m - d, -180, 180)) <= c))
          ) {
            k = a.lat();
          ...
          if (K - H > 180 ? d + c >= K || d - c <= H : d + c >= H && d - c <= K)
            if (((h = $oa(h, F, d, f, b)), Math.abs(h - a.lat()) < c))
              return !0;
        }
      }
      return !1;
    ...
    gpa = function (a, b) {
      ...
      for (let v = 0; v < a.length; ++v)
        (epa(a[v], d), YC(f, d, c), fpa(c, e, e), ([f[0], f[1], f[2]] = d));
      const [g, h, k] = f,
        [m, p, r, t] = e;
    ...
    ZC = class {};
  ZC.isLocationOnEdge = bpa;
  ZC.containsLocation = function (a, b) {
    a = _.on(a);
  ...
  var $C = class {};
  $C.computeSignedArea = gpa;
  $C.computeArea = function (a, b) {
    if (
      !(
        a instanceof _.xp ||
        Array.isArray(a) ||
    ...
    return Math.abs(gpa(a, b));
  };
  $C.computeLength = function (a, b) {
    b = b || 6378137;
    let c = 0;
    a instanceof _.xp && (a = a.getArray());
    for (let d = 0, e = a.length - 1; d < e; ++d) c += dpa(a[d], a[d + 1], b);
    return c;
  };
  $C.computeDistanceBetween = dpa;
  ...
  $C.computeOffset = function (a, b, c, d) {
    ...
    return new _.hn(
      _.ql(Math.asin(g)),
      _.ql(a + Math.atan2(b * e * Math.sin(c), d - f * g)),
    );
  };
  $C.computeHeading = function (a, b) {
    a = _.on(a);
    b = _.on(b);
    const c = _.kn(a),
      d = _.ln(a);
  ...
  var ipa = { encoding: _.Ru, spherical: $C, poly: ZC };
  _.pa.google.maps.geometry = ipa;
  _.Ml("geometry", ipa);
});
